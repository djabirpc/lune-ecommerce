...
- **Storefront design refresh** (CLAUDE.md sections 3/6/7/8 — mobile-first, premium/modern/minimal): new `Fraunces` (display, italic) + `Inter` (body) font pairing loaded in `index.html`; `index.css` gained `--color-luna-accent-dark`/`--color-luna-rose` tints alongside the existing (previously undocumented) `--color-luna-accent`. `StorefrontLayout` rebuilt with a sticky header, a mobile hamburger drawer menu (there was previously **no mobile navigation at all** — nav links just squeezed inline), a real cart icon with a badge (was plain "Panier (N)" text), and a proper multi-column footer. `HomePage` went from 2 static sections to a real data-driven page: hero, trust strip, a **Promotion banner** (highest-priority active promotion), a **Categories** grid, **Nouveautés** (real newest products), and a conditional **"Suivez-nous"** section (only rendered when social URLs are configured). Every storefront page (`ProductCard`, `CategoryPage`, `CategoriesPage`, `ProductPage`, `CartPage`, `CheckoutPage`, `OrderConfirmationPage`, `TrackOrderPage`, `PromotionsPage`, `OrderDetailsCard`) restyled to match. `ProductPage` and `CartPage` gained a **sticky mobile bottom CTA bar** (fixed, always-reachable primary action) — a real mobile-UX change, not just visual. Admin UI was **not** touched (storefront-only pass).
- `GET /api/products` gained an optional `sortByNewest` query param (`ProductService.GetPagedAsync`) — additive, orders by `CreatedAtUtc` descending instead of the default alphabetical order; powers the homepage's Nouveautés section with real data.
- **"Atlas Express" fake-carrier rebrand**: `FakeShippingProvider` (unchanged behavior — still the CLAUDE.md-mandated dev/test double) is now presented as the fictional carrier "Atlas Express" everywhere the UI shows it: `SHIPPING_CARRIER_LABELS.Fake`, the tracking-number prefix (`ATL` instead of `FK`), the provider-shipment-ID prefix, the tracking-sync description, and the placeholder shipment label. Purely cosmetic — the label is still explicitly stamped as a development-only simulation.
...
- `GET /api/products?sortByNewest=` param added (see Added) — additive, existing callers unaffected.
...
- No migration for the storefront redesign, `sortByNewest`, or the Atlas Express rebrand — no schema changes in any of the three.
...
- Full storefront visual redesign (see Added) — every customer-facing page restyled, mobile navigation added, new homepage sections.

### Notes
- Full backend test suite (`dotnet test` from `backend/`) passes: 117/117 tests (49 Application.Tests, 68 Api.Tests), re-verified after the `sortByNewest` addition (1 new `CatalogEndpointsTests` test).
- `npm run build` (`tsc -b && vite build`) passes with no type errors.
- The storefront redesign was verified with a full headless-browser walkthrough against the live Docker stack, seeding realistic demo data (3 categories, 6 products with real uploaded photos, a storewide promotion) and driving the entire customer + fulfillment path at both 1440px and CLAUDE.md's 390px mobile target: home → mobile menu → category → product (variant selection, sticky mobile CTA) → cart → checkout (live shipping quote + coupon) → order confirmation → order tracking → admin order detail → shipment created via Atlas Express → tracking synced to completion → `/admin/shipping` correctly showing only Atlas Express available. Screenshots from this run were compiled into a shareable walkthrough. Two Playwright locator gotchas hit and fixed along the way (not app bugs, see PROJECT_CONTEXT.md Known Issues): a case-insensitive `has-text("S")` matched the "Rose" color button instead of the size button, and a bare text-based wait resolved to a `display:none` desktop-only CTA duplicate instead of the visible mobile one.
