...
frontend/src/
  ...
  admin/        — back-office layout + pages (AdminLayout wraps "/admin"); LoginPage, DashboardPage, OrdersPage, OrderDetailPage, OrderConfirmationCenterPage, ProductsPage are real; the rest still PagePlaceholder
  ...
  lib/format/   — formatPrice (DZD), ORDER_STATUS_LABELS/DELIVERY_TYPE_LABELS/CALL_ATTEMPT_RESULT_LABELS, slugSchema (Zod, mirrors the backend's slug regex — used by both CategoryQuickManager and CreateProductForm)
...
None in progress. Auth, Catalog, Inventory, core Order/COD workflow, the customer-facing storefront purchase journey, admin authentication + order + product/category/inventory management, and now the confirmation-center workflow are complete. Next up: Promotions — see Next Recommended Steps.

## Last Completed Work

2026-09-01 — Confirmation-center workflow (CLAUDE.md section 13): `OrderCallAttempt` entity + migration, `IOrderCallAttemptService`/`OrderCallAttemptService` (delegates `Confirmed`/`Cancelled` results to the existing `OrderService.ChangeStatusAsync` rather than duplicating transition logic), `POST /api/orders/{id}/call-attempts`, a real `/admin/orders/confirmation` page (two live queues), and a call-log form + journal on `OrderDetailPage`. Backend test suite grew to 53/53 (34 unit + 19 integration, including 4 new `OrderCallAttemptTests` covering the NoAnswer/Confirmed/CallbackScheduled-validation/non-callable-order cases). Verified with a full headless-browser session against the live Docker stack (see the Confirmation-center bullet above for the exact flow). One test-tooling gotcha hit and fixed along the way: Chromium's `element.innerText` reflects CSS `text-transform` (a heading styled `uppercase` reads back as literal uppercase text), so a case-sensitive JS string check against it silently never matches — noted in Known Issues.
...
Migrations: `backend/src/Ecommerce.Infrastructure/Persistence/Migrations/` — `InitialIdentity`, `AddRefreshTokens`, `AddCatalogAndInventory`, `AddOrders`, `AddOrderCallAttempts` (all applied and verified against a live Postgres container).

Planned entities (not yet created): Customer, CustomerAddress, Promotion, Shipment, TrackingEvent, ShippingRate, MarketingEvent, AuditLog. No Cart/CartItem entity is planned — see Important Decisions.
...
**Admin status**: `LoginPage`, `DashboardPage` (real order counts), `OrdersPage` (real, paged/filterable), `OrderDetailPage` (real, with status-transition actions + history + call-attempt form/journal), `OrderConfirmationCenterPage` (real, two live queues), and `ProductsPage` (real: category management, product creation, expandable per-product stock panel with inline restock/adjust) are wired to the backend and protected by `RequireAdminAuth`. `InventoryPage` (route still resolves, but stock management lives on `ProductsPage` instead — see Important Decisions), `PromotionsPage`, `CustomersPage`, `ShippingPage`, `MarketingPage`, `UsersPage`, `SettingsPage` remain `PagePlaceholder`.
...
12. Implement admin (real data, not placeholders) — login, Orders (incl. Confirmation Center), and Products/Categories/Inventory management done; Promotions/Customers/Shipping/Marketing/Users/Settings admin pages remain `PagePlaceholder`.
...
1. Promotions (CLAUDE.md section 20) — no dependency on anything unbuilt, could be picked up independently of the shipping/marketing work below.
2. Shipping: `IShippingProvider` + `FakeShippingProvider` abstraction (CLAUDE.md section 15), needed before `Order.ShippingCost` can be anything other than 0, and before Yalidine/ZR Express integration (both still blocked on real API docs/credentials per CLAUDE.md sections 16–17 — do not invent endpoints when picking this up).
3. Marketing tracking (pixels, UTM capture, attribution) — lowest priority until there's checkout traffic to actually track.
4. Smaller, can be done anytime: real product photography/images (`ProductImage`) — every product created so far has none, so the storefront always shows the "Pas d'image" placeholder box; not a bug, just no image upload flow or seeded image URLs yet. No admin UI to add images exists yet either.
5. Smaller, can be done anytime: `/admin/users` management (currently `PagePlaceholder`) — the only way to create staff accounts today is the `InitialAdmin` bootstrap seed (Important Decisions #12); there's no backend endpoint for it yet either, so this needs backend work too, not just UI.
6. Smaller, can be done anytime: product/category `PUT` (update) UI — the backend endpoints exist (`PUT /api/products/{id}`, `PUT /api/categories/{id}`) but `ProductsPage` only wires up create so far; deactivating a product currently requires a direct API call.
