...
frontend/src/
  ...
  admin/        — back-office layout + pages (AdminLayout wraps "/admin"); LoginPage, DashboardPage, OrdersPage, OrderDetailPage are real; the rest still PagePlaceholder
  lib/api/      — fetch-based apiClient (auth-aware, see below) + TanStack QueryClient + types.ts (hand-written, mirrors backend DTOs) + catalog.ts/orders.ts/auth.ts (API functions)
...
`types.ts` is hand-written, not generated from the backend — if a backend DTO shape changes, this file must be updated manually (no codegen pipeline exists yet). `lib/orders/transitions.ts` has the same hand-sync risk, called out explicitly in its own file comment.
...
- Admin route shell (`/admin/*`) with separate layout/navigation, now with a **working login + protected routes** (see the Orders admin bullet below) — product/inventory/promotions/customers/shipping/marketing/users/settings pages remain `PagePlaceholder`.
...
None in progress. Auth, Catalog, Inventory, core Order/COD workflow, the customer-facing storefront purchase journey, and now admin authentication + order management are complete. Next up: remaining admin pages (products/categories/inventory management, confirmation-center) — see Next Recommended Steps.

## Last Completed Work

2026-09-01 — Admin authentication and order management: login page, token storage with automatic attach + refresh-on-401 (including concurrent-401 coordination so the rotating refresh token isn't double-spent), a route guard protecting all `/admin/*` pages, a real (not placeholder) orders list/detail/status-change UI respecting the backend's valid-transitions map, and a dashboard showing real order counts. One small backend addition: `OrderDetailDto` now carries `StatusHistory`, populated only for the admin `GetByIdAsync` call (never for guest-facing create/track, to avoid leaking internal transition reasons). Verified with a full headless-browser session against the Dockerized stack — login redirect enforcement, dashboard stats, filtered order list, a real status transition with the history entry appearing, and session teardown on logout. Backend test suite (43/43) re-verified unaffected by the DTO change.

Earlier same-date milestones: storefront frontend wired to the real backend (category/product browsing, cart, checkout, confirmation, tracking — also browser-verified), Orders & COD checkout (state machine + inventory integration, both CLAUDE.md section 29 test scenarios), Catalog & Inventory (`Category`/`Product`/`ProductVariant`/`ProductImage`, `InventoryRecord`/`InventoryTransaction`, atomic never-oversell stock mutations), authentication (JWT login/refresh/logout/me, role + bootstrap SUPER_ADMIN seeding, standardized error responses — and a real eager-`IConfiguration`-read bug fixed along the way), and the original technical bootstrap (monorepo scaffold, backend/frontend skeletons, Docker Compose stack). See `CHANGELOG.md` for full bullet lists of each.
...
**Admin status**: `LoginPage`, `DashboardPage` (real order counts), `OrdersPage` (real, paged/filterable), `OrderDetailPage` (real, with status-transition actions + history) are wired to the backend and protected by `RequireAdminAuth`. `ProductsPage`, `InventoryPage`, `PromotionsPage`, `CustomersPage`, `ShippingPage`, `MarketingPage`, `UsersPage`, `SettingsPage` remain `PagePlaceholder` (reachable once logged in, but show no real data yet).
...
12. Implement admin (real data, not placeholders) — login + Orders management done; Products/Categories/Inventory/Promotions/Customers/Shipping/Marketing/Users/Settings admin pages remain `PagePlaceholder`.
...
1. Admin Products/Categories/Inventory management UI (`/admin/products`, `/admin/inventory`, currently `PagePlaceholder`) — the backend endpoints (`CategoriesController`/`ProductsController`/`InventoryController`) are ready and already exercised via curl; this is now the main remaining "backend exists, UI doesn't" gap, needed so staff don't have to use curl/Swagger to add products or restock.
2. `OrderCallAttempt` + confirmation-center workflow (CLAUDE.md section 13), now that the admin order UI exists to actually use it (`/admin/orders/confirmation` is still `PagePlaceholder`) — see Important Decisions #19.
...
7. Smaller, can be done anytime: `/admin/users` management (currently `PagePlaceholder`) — the only way to create staff accounts today is the `InitialAdmin` bootstrap seed (Important Decisions #12); there's no backend endpoint for it yet either, so this needs backend work too, not just UI.
